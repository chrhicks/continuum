...
import { consolidateNow, dedupeEntriesByAnchor, insertEntryInSection } from "../src/memory/consolidate";
...
describe("memory index de-duplication", () => {
  ...
  test("insertEntryInSection skips entries with duplicate anchors", () => {
    const existing = "- **[Session 2026-02-02 02:00](MEMORY-2026-02-02.md#session-abc)** - Session work";
    const content = [
    ...
    const additional = "- **[Session 2026-02-02 02:01](MEMORY-2026-02-02.md#session-def)** - Session work";
    const updated = insertEntryInSection(content, "Sessions", additional);
    const sessionLines = updated.split("\n").filter((line) => line.includes("#session-"));
  ...
  test("consolidateNow removes duplicate anchors already in MEMORY.md", () => {
    withTempCwd(() => {
      ...
      const matches = updatedIndex.match(new RegExp(`#${duplicateAnchor}`, "g")) ?? [];

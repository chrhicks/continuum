...
describe("memory search CLI", () => {
  test("accepts --tier=VALUE syntax", async () => {
    await withTempCwd(async () => {
      ...
      writeFileSync(join(memoryDir, "NOW-2026-02-02T16-10-00.md"), "hello world", "utf-8");

      const originalArgv = process.argv;
      process.argv = ["node", "continuum", "memory", "search", "hello", "--tier=NOW"];
  ...
  test("filters results with --tags", async () => {
    await withTempCwd(async () => {
      ...
      const originalArgv = process.argv;
      process.argv = ["node", "continuum", "memory", "search", "hello", "--tags", "alpha"];

      try {
        const logs = await withCapturedLogs(async () => {
          await main();
        });
        expect(logs.some((line) => line.includes("NOW-2026-02-02T16-10-00-alpha.md"))).toBe(true);
        expect(logs.some((line) => line.includes("NOW-2026-02-02T16-10-00-beta.md"))).toBe(false);
      } finally {
        process.argv = originalArgv;
      }
    });
...
describe("memory session CLI", () => {
  test("end --consolidate triggers consolidation", async () => {
    await withTempCwd(async () => {
      ...
      const originalArgv = process.argv;
      process.argv = ["node", "continuum", "memory", "session", "end", "--consolidate"];

      try {
        const logs = await withCapturedLogs(async () => {
          await main();
        });
        expect(logs.some((line) => line.startsWith("Session ended:"))).toBe(true);
        expect(logs.some((line) => line.startsWith("Consolidation complete:"))).toBe(true);
      } finally {
        process.argv = originalArgv;
      }
    });

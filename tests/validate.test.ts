...
describe('validateMemory', () => {
  test('flags missing frontmatter keys', () => {
    withTempMemoryDir((memoryDir) => {
      ...
      expect(
        messages.some((message) => message.includes('Missing frontmatter key')),
      ).toBe(true)
    })
  })
  ...
  test('flags missing anchors referenced by MEMORY.md', () => {
    withTempMemoryDir((memoryDir) => {
      ...
      expect(
        messages.some((message) => message.includes('Missing anchor')),
      ).toBe(true)
    })
  })
    ...
    withTempMemoryDir((memoryDir) => {
      ...
      expect(
        messages.some((message) =>
          message.includes('Invalid frontmatter value'),
        ),
      ).toBe(true)
    })
  })

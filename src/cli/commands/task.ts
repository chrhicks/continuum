...
export function createTaskCommand(): Command {
  const taskCommand = createTaskRootCommand()
  registerTaskListCommand(taskCommand)
  registerTaskGetCommand(taskCommand)
  registerCrudCommands(taskCommand)
  registerTaskValidateCommand(taskCommand)
  registerTaskGraphCommand(taskCommand)
  registerTaskTemplatesCommand(taskCommand)
  registerStepsCommands(taskCommand)
  registerNoteCommands(taskCommand)
  return taskCommand
}

function createTaskRootCommand(): Command {
...
function registerTaskListCommand(taskCommand: Command): void {
  taskCommand
    .command('list')
    ...
    .action(handleTaskListAction)
}

async function handleTaskListAction(
...
function registerTaskGetCommand(taskCommand: Command): void {
  taskCommand
    .command('get')
    .alias('view')
...
async function handleTaskGetAction(
  taskId: string,
  ...
  await runCommand(
    command,
    () => loadTaskGetResult(taskId, options),
    (result) => printTaskGetResult(result, options),
  )
}

async function loadTaskGetResult(
  taskId: string,
  options: TaskGetOptions,
...
function registerTaskValidateCommand(taskCommand: Command): void {
  taskCommand
    .command('validate')
    .description('Validate a status transition')
    .argument('<task_id>', 'Task ID to validate')
    .option('--transition <status>', 'Target status')
    .action(handleTaskValidateAction)
}

async function handleTaskValidateAction(
  taskId: string,
  ...
  await runCommand(
    ...
    async () => {
      ...
      const result = await continuum.task.validateTransition(taskId, status)
...
function registerTaskGraphCommand(taskCommand: Command): void {
  taskCommand
    .command('graph')
    ...
    .action(handleTaskGraphAction)
}

async function handleTaskGraphAction(
  query: string,
  taskId: string,
  _options: unknown,
  command: Command,
...
function registerTaskTemplatesCommand(taskCommand: Command): void {

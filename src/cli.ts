...
function handleSearch(args: string[]): void {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg.startsWith('--tier=')) {
      ...
      if (!value) {
        throw new Error(
          'Missing tier. Use: continuum memory search <query> --tier NOW|RECENT|MEMORY|all',
        )
      }
...
function handleLog(args: string[]): void {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--tail') {
      ...
      if (!value) {
        throw new Error(
          'Missing tail count. Use: continuum memory log --tail <lines>',
        )
      }
      tail = parseTail(value)
      i += 1
...
function handleRecover(args: string[]): void {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--hours') {
      ...
      if (!value) {
        throw new Error(
          'Missing hours. Use: continuum memory recover --hours <hours>',
        )
      }
      maxHours = parseHours(value)
      i += 1
...
function printRootHelp(): void {
  ...
  console.log(
    '  continuum memory search <query> [--tier NOW|RECENT|MEMORY|all] [--tags tag1,tag2]',
  )
  ...
  console.log('Markers:')
  console.log(
    '  Use @decision, @discovery, @pattern in NOW to extract highlights.',
  )
}
...
function parseTaskListOptions(args: string[]): {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--status' || arg === '-s') {
      ...
      if (!value) {
        throw new Error(
          'Missing status. Use: continuum task list --status <status>',
        )
      }
      status = parseTaskStatus(value)
      i += 1
...
function printAppendHelp(): void {
  ...
  console.log(
    '  (Use /exit or /exit --consolidate in user messages to end the session.)',
  )
}

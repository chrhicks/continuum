...
export function scanStaleNowFiles(
  ...
  const nowFiles = readdirSync(memoryDir).filter((fileName) => /^NOW-.*\.md$/.test(fileName));
...
export function recoverStaleNowFiles(options: { maxHours?: number; consolidate?: boolean } = {}): RecoverResult {
...
function resolveStartTimestamp(filePath: string): { timestampStart: string | null; startMs: number } {
  const content = readFileSync(filePath, "utf-8");
  const { frontmatter } = parseFrontmatter(content);
  const raw = frontmatter.timestamp_start ? String(frontmatter.timestamp_start) : null;
  if (raw) {

...
export function validateMemory(options: { memoryDir?: string } = {}): MemoryValidationResult {
  ...
  for (const filePath of files) {
    ...
    if (fileName.startsWith("NOW-")) {
      errors.push(...validateFrontmatter(filePath, NOW_REQUIRED_KEYS, "NOW", NOW_SCHEMA));
      continue;
    }
    if (fileName.startsWith("MEMORY-")) {
      errors.push(...validateFrontmatter(filePath, MEMORY_REQUIRED_KEYS, undefined, MEMORY_SCHEMA));
    }
  }
  ...
  return { errors, filesChecked: files.length + (existsSync(indexPath) ? 1 : 0) };
}
...
function validateFrontmatter(
  ...
  if (schema) {
    errors.push(...validateFrontmatterSchema(filePath, frontmatter, keyLines, startLine, schema));
  }
...
function validateIndexLinks(indexPath: string, memoryDir: string): MemoryValidationError[] {

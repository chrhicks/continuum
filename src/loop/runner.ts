...
export async function runLoopRequest(requestPath?: string): Promise<LoopRunnerResult> {
  const path = requestPath ?? join(".continuum", "loop", "request.json");
  if (!existsSync(path)) {
    return { invoked: false, message: "No loop request found.", requestPath: path };
  }
  ...
  if (!runner) {
    return {
      invoked: false,
      message: "Agent loop skill tool unavailable. Request saved for external processing.",
      requestPath: path
    };
  }
  try {
    ...
    unlinkSync(path);
    return { invoked: true, message: "Agent loop skill invoked.", requestPath: path };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return {
      invoked: false,
...
function resolveSkillRunner(): SkillRunner | null {
  const candidate = (globalThis as { opencode?: { runSkill?: SkillRunner }; runSkill?: SkillRunner }).opencode
    ?.runSkill ?? (globalThis as { runSkill?: SkillRunner }).runSkill;
